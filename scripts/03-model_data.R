...
model2 <- lm(pct ~ pollster + sample_size + state, data = harris_data)
saveRDS(model2, here("models", "model2.rds"))

# Model 3: Adding recency as a predictor
model3 <- lm(pct ~ pollster + sample_size + state + recency, data = harris_data)
...
model5 <- lm(pct ~ pollster + sample_size + state, data = trump_data)
saveRDS(model5, here("models", "model5.rds"))

# Model 6: Adding recency as a predictor
model6 <- lm(pct ~ pollster + sample_size + state + recency, data = trump_data)
