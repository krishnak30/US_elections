...
# Create model summary data for Harris
modelsumharris <- harris_data %>%
  mutate(
    fit = predict(model3, newdata = ., interval = "confidence"),
    lower = fit[, "lwr"],
    upper = fit[, "upr"],
    fitted = fit[, "fit"]
  ) %>%
  select(end_date, fitted, lower, upper, state)  # Include state column

# Create model summary data for Trump
modelsumtrump <- trump_data %>%
  mutate(
    fit = predict(model6, newdata = ., interval = "confidence"),
    lower = fit[, "lwr"],
    upper = fit[, "upr"],
    fitted = fit[, "fit"]
  ) %>%
...
modelsumharris$candidate_name <- "Kamala Harris"
modelsumtrump$candidate_name <- "Donald Trump"
combined_data <- bind_rows(modelsumharris, modelsumtrump)
```
```{r}
...
ggplot(combined_data, aes(x = as.Date(end_date), y = fitted, color = candidate_name, fill = candidate_name)) +
  geom_smooth(method = "loess", se = TRUE, span = 0.2) +
  scale_color_manual(values = c("Kamala Harris" = "steelblue", "Donald Trump" = "indianred")) +
  scale_fill_manual(values = c("Kamala Harris" = "steelblue", "Donald Trump" = "indianred")) +
  labs(x = "Date", y = "Support (%)") +
...
#| label: fig-modsum2
#| eval: true 
#| fig-cap: 'Model summary of Donald Trump’s support trends across key battleground states.'
# Define the target states
target_states <- c("Arizona", "Georgia", "Michigan", "Pennsylvania", "Wisconsin", "North Carolina", "Nevada")

# Filter combined_data for Donald Trump, exclude national data, and limit to target states
trump_state_data <- combined_data %>%
  filter(candidate_name == "Donald Trump", state %in% target_states)

# Plotting
ggplot(trump_state_data, aes(x = as.Date(end_date), y = fitted, fill = candidate_name)) +
  geom_smooth(method = "loess", se = TRUE, span = 0.2, color = "indianred") +
  scale_fill_manual(values = c("Donald Trump" = "indianred")) +
  labs(x = "Date", y = "Support (%)",
       title = "Support trends for Donald Trump across different states") +
  theme_minimal() +
  theme(
  ...
  facet_wrap(~ state, scales = "free_y")  # Separate subplots for each state
...
#| message: false
#| label: fig-modsum1
#| fig-cap: 'Model summary of Kamala Harris’s support trends across key battleground states.'
# Define the target states
target_states <- c("Arizona", "Georgia", "Michigan", "Pennsylvania", "Wisconsin", "North Carolina", "Nevada")

# Filter combined_data for Kamala Harris and limit to target states
harris_state_data <- combined_data %>%
  filter(candidate_name == "Kamala Harris", state %in% target_states)

# Plotting
ggplot(harris_state_data, aes(x = as.Date(end_date), y = fitted, fill = candidate_name)) +
  geom_smooth(method = "loess", se = TRUE, span = 0.2, color = "steelblue") +
  scale_fill_manual(values = c("Kamala Harris" = "steelblue")) +
  labs(x = "Date", y = "Support (%)",
       title = "Support trends for Kamala Harris across different states") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    legend.title = element_blank(),
...
The target population is the whole group of interest the researchers would like to make conclusions on [@def_terms]. For the poll conducted by ECP in October, the target population is likely voters in the U.S. elections [@ecp_poll]. Choosing likely voters as the target population makes the poll results more accurate as not all registered voters end up voting in the Presidential Election. The last U.S. Presidential Election in 2020 only had 66.8% of registered voters participate in the elections, which is the highest voter turnout in the 21st century [@us_census]. In ECP’s methodology, likely voters are determined by a combination of voter history, registration status, and demographic data. These factors are self-reported [@ecp_poll].
...
The Emerson survey is designed with simplicity and efficiency. One of its strengths is its straightforward format, using unambiguous questions. This simplicity makes the survey easy for respondents to understand. Another strength is its topical relevance. By focusing on vote preferences and demographic splits, the survey provides valuable information during election cycles. However, the survey has some limitations. One is the lack of deeper understanding; while it captures basic voter preferences, it doesn’t explore the motivations or factors driving those choices in depth. Additionally, the various survey modes can affect respondent engagement with the questions, which may also influence the accuracy and depth of their answers.
